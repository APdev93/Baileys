...
export const generateProfilePicture = async (mediaUpload: WAMediaUpload) => {
	...
	if ("sharp" in lib && typeof lib.sharp?.default === "function") {
		img = lib.sharp
			...
			.png({ quality: 50 }) // Gunakan PNG untuk mendukung transparansi
			.toBuffer();
	} else if ("jimp" in lib && typeof lib.jimp?.read === "function") {
		const { read, MIME_PNG, RESIZE_BILINEAR } = lib.jimp;
		const jimp = await read(bufferOrFilePath as string);
		const min = Math.min(jimp.getWidth(), jimp.getHeight());
		const cropped = jimp.crop(0, 0, min, min);

		img = cropped.quality(50).resize(640, 640, RESIZE_BILINEAR).getBufferAsync(MIME_PNG); // Gunakan PNG untuk mendukung transparansi
	} else {
		throw new Boom("No image processing library available");
	}
...
export async function getAudioWaveform(buffer: Buffer | string | Readable, logger?: Logger) {
	try {
		const audioDecode = (buffer: Buffer | ArrayBuffer | Uint8Array) =>
			import("audio-decode").then(({ default: audioDecode }) => audioDecode(buffer));
