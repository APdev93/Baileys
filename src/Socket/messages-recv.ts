...
export const makeMessagesRecvSocket = (config: SocketConfig) => {
                    ...
                    content: [
                        {
                            tag: "type",
                            attrs: {},
                            content: Buffer.from(KEY_BUNDLE_TYPE),
                        },
                        {
                            tag: "identity",
                            attrs: {},
    ...
    const handleGroupNotification = (
        ...
        switch (child?.tag) {
            ...
            case "not_ephemeral":
                ...
                break;
            case "modify":
                const oldNumber = getBinaryNodeChildren(
                    child,
                    "participant"
                ).map((p) => p.attrs.jid);
    ...
    const processNotification = async (node: BinaryNode) => {
        ...
        switch (nodeType) {
            ...
            case "link_code_companion_reg":
                ...
                const linkCodePairingExpanded = await hkdf(
                    companionSharedKey,
                    32,
                    {
                        salt: linkCodeSalt,
    ...
    const fetchMessageHistory = async (
        ...
        const pdoMessage: proto.Message.IPeerDataOperationRequestMessage = {
            ...
            peerDataOperationRequestType:
                proto.Message.PeerDataOperationRequestType
    ...
    const handleCall = async (node: BinaryNode) => {
        ...
        if (status === "offer") {
            ...
            call.isGroup =
                infoChild.attrs.type === "group" ||
                !!infoChild.attrs["group-jid"];
